...
<html>
  ...
  <body>
		...
		<script type="text/html" id="conTemp">
			...
			<tr>
				<td>{{ item.id }}</td>
				<td>{{ item.kindOutput }}</td>
                <td> 
                {{# if(item.pic){ }}
                    <img alt="" src="${domain_img }{{ item.pic }}" style="width:80%;">
                {{# } }}
                </td>
				<td>{{ item.href }}</td>
                <td>{{ item.publishFlagOutput }}</td>
					...
					complate: function() { //完成的回调
						...
						$('a[data-opt="del"]').on('click', function() {
							...
							layer.confirm('确定该操作吗？', {icon: 3, title:'提示'}, function(index){
								$.post('/banner/deletebyid.json', {"id": id}, function(res){
